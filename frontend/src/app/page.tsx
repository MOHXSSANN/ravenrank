...
function getStats() {
  ...
  const professors = db.prepare("SELECT COUNT(*) as count FROM professors").get() as { count: number };
...
function getPopularSubjects() {
  ...
  const subjects = db
    ...
    .all() as Array<{ code: string; title: string; faculty: string; course_count: number }>;
  ...
  return subjects.map((subject) => {
    const grades = db
      ...
      .get(subject.code) as GradeDistribution | null;

    return { ...subject, grades };
  });
}

function getTopProfessors() {
  const db = getDb();
  return db
    .prepare(
...
export default function Home() {
  ...
  const topProfessors = getTopProfessors();
